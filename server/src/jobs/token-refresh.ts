...
async function scheduleFromConfig(reason: 'startup' | 'update' | 'completed' | 'retry'): Promise<void> {
    ...
    if (!state.enabled) {
        logger.info({ systemEvent: true, action: 'token_refresh.auto_disabled', trigger: 'AUTO', reason }, 'Auto refresh scheduler disabled');
        await persistAndSchedule(null);
        return;
    }

    if (reason === 'update' && tokenRefreshService.isAutoRunInProgress()) {
        logger.info({ systemEvent: true, action: 'token_refresh.auto_update_deferred', trigger: 'AUTO', reason }, 'Auto refresh config update deferred, waiting for current auto job to finish');
        return;
    }
...
async function executeScheduledRun(): Promise<void> {
    ...
    try {
        const state = await tokenRefreshService.getTokenRefreshScheduleState();
        if (!state.enabled) {
            logger.info({ systemEvent: true, action: 'token_refresh.auto_skip_disabled', trigger: 'AUTO' }, 'Auto refresh skipped because the scheduler is disabled');
            return;
        }
        ...
        if (tokenRefreshService.isRefreshRunning()) {
            ...
            }, 'Auto refresh deferred because another refresh job is currently running');
            return;
        }
        ...
        nextReason = 'retry';
        logger.error({ err, systemEvent: true, action: 'token_refresh.auto_failed', trigger: 'AUTO' }, 'Auto refresh job failed');
    } finally {
        if (!stopped) {
